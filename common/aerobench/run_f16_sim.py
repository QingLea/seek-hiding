...
from gcas.gcas_autopilot import GcasAutopilot

from common.aerobench.highlevel.controlled_f16 import controlled_f16
from common.aerobench.util import get_state_names, Euler
...
def gcas_simulation(phi, theta, psi, initial_altitude=2330):
    # The code below is adapted from AeroBenchVVPython/v2/code/aerobench/examples/gcas/run_GCAS.py
    # phi  Roll angle from wings level (rad), valid range [0.2 * pi, 0.2833 * pi]
    # theta Pitch angle from nose level (rad), valid range  [-0.4 * pi, -0.35 * pi]
    # psi Yaw angle from North (rad), valid range [-0.375 * pi, -0.125 * pi]

    assert in_range(phi, [0.2 * math.pi, 0.2833 * math.pi])
    assert in_range(theta,  [-0.4 * math.pi, -0.35 * math.pi])
    assert in_range(psi,[-0.375 * math.pi, -0.125 * math.pi])
    ...
    # Initial Attitude
    alt = initial_altitude  # altitude (ft)
...
# Example of use
# result=gcas_simulation(0.21 * math.pi, -0.38 * math.pi, -0.35 * math.pi)
# 
# print(result)
